...
class sem_panels {
	...
	**/
	...
	function register_sidebar_panels() {
		...
		if ( strpos($sem_theme_options['active_layout'], 't') !== false ) {
			register_sidebar(
				array(
					'id' => 'top_sidebar',
					'name' => __('Top Wide Sidebar', 'sem-pinnacle'),
					'before_widget' => $before_widget,
					'after_widget' => $after_widget,
					'before_title' => $before_title,
					'after_title' => $after_title,
			...
			register_sidebar(
				array(
					'id' => 'sidebar-1',
					'name' => __('Sidebar 1/Leftmost', 'sem-pinnacle'),
					'before_widget' => $before_widget,
					'after_widget' => $after_widget,
					'before_title' => $before_title,
					'after_title' => $after_title,
			...
			register_sidebar(
				array(
					'id' => 'sidebar-2',
					'name' => __('Sidebar 2/Rightmost', 'sem-pinnacle'),
					'before_widget' => $before_widget,
					'after_widget' => $after_widget,
					'before_title' => $before_title,
					'after_title' => $after_title,
			...
			register_sidebar(
				array(
					'id' => 'bottom_sidebar',
					'name' => __('Bottom Wide Sidebar', 'sem-pinnacle'),
					'before_widget' => $before_widget,
					'after_widget' => $after_widget,
					'before_title' => $before_title,
					'after_title' => $after_title,
	...
	function init_widgets() {
		...
		if ( is_admin() ) {
			...
			if ( empty($_GET['preview']) && empty($_GET['stylesheet']) ) {
				...
				update_option('upgrade_sempinnacle_panels', '0');
			}
		} else {
	...
	/**
	 * upgrade_sem_reloaded_widgets()
	 *
	 * @return void
	 **/
	function upgrade_sem_pinnacle_widgets() {
		...
		if ( !is_active_widget(false, false, 'entry_footer') ) {
			...
			if ( $key !== false )
				array_splice( $entry_widgets, $key, 0, (array) $widget_id );
			else
				array_push($entry_widgets, $widget_id);
		}
